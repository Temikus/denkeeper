...
- **Supervisor review spend is billed per conversation, to the supervisor's agent** (`supervisorSessionKey`, which carries the reasoning): the key is `supervisor:{reviewed}:{convID}` and is registered with the cost tracker under the supervisor's own name. Both halves are load-bearing and neither is inferable from the call site — a process-constant key makes the per-session `cost_limit_hard` a lifetime budget that nothing resets (every review then fails closed to a human until restart), and skipping `RegisterSessionAgent` bills a phantom agent named `supervisor`, which also skips the real agent's configured limits. A failed review records `detail.cause` (`cost_limit`/`timeout`/`provider_error`) since all causes land on the same `supervisor_error` status.
