...
| `POST eval/runs/{id}/judge` | `eval:write` | starts the internal judge (`[eval] judge_model`) over the run's pending queue and returns `202` with the item count; `503` when no judge model is configured, `409` when the run is not terminal or a pass is already running. `eval:write` rather than `eval:read` because it spends money. It never widens the judge surface: the pass reads only `GetBlindedItem`, so the REST-only unblinded `/pairs` view stays out of reach |
| `POST eval/estimate`, `GET eval/config`, `GET eval/suggest`, `GET eval/probes` | `eval:read` (`probes` also needs `agents:read`) | estimate mirrors the run body, basis `history`/`list_price`/`unknown` and never fabricates; `config` is read-only by design (no runtime threshold writer); `suggest` is stratified across the four **history** categories, skips already-saved turns, `501` without telemetry; `probes` is the top-down counterpart — required `?agent=`, generated from tier/auto-approve/persona/skills into `category=probe`, `?set=` excludes what that set already holds, and generation stays inside the caller's scopes (403 without `agents:read`; skill family dropped without `skills:read`, approval family without `tools:read`) |
