...
- **Chat**: `POST /api/v1/chat` (scope `chat`) — JSON body `{agent, session_id, message, user_id, user_name}`; returns `{session_id, response}`. Set `Accept: text/event-stream` for SSE streaming: emits `{"type":"thinking","text":"..."}` before each LLM call, `{"type":"tool_start","tool":"...","round":N}` and `{"type":"tool_end","tool":"...","round":N,"duration_ms":N}` during agentic tool loops, `{"type":"tool_approval","tool":"...","round":N,"approval_id":"...","approval_callback":"appr:..."}` when a supervised tool call is pending, `{"type":"usage","tokens":N,"cost_usd":F}` after the response completes, then `{"type":"content","text":"..."}` and `{"type":"done","session_id":"..."}`. `session_id` is auto-generated if omitted; pass the same value in subsequent requests to continue the conversation.
- **Models**: `GET /api/v1/models` (scope `agents:read`) — returns `{models: [...]}` with a sorted list of available model IDs from all configured providers (Anthropic, OpenAI, OpenRouter, Ollama). Returns 503 if no provider supports listing.
...
Seven action kinds: `user_update`, `soul_update`, `create_skill`, `modify_schedule`, `install_tool`, `browser_profile`, `tool_call`. Default TTL: 24 h (background worker ticks hourly).

**Supervised tool call approval**: When `permission_tier = "supervised"`, each MCP tool call the LLM requests is submitted to the approval Manager before execution. The engine calls `Manager.WaitForResolution(ctx, id)` which blocks until the operator approves or denies. The Dispatcher's event handler intercepts `"tool_approval"` ChatEvents and sends an intermediate message with inline Approve/Deny keyboard buttons. If denied, the LLM receives "Tool call was denied by the operator." as the tool result. If context is cancelled (e.g. timeout), the approval is treated as expired.
...
Available MCP tools: `list_skills`, `create_skill`, `skill_get`, `skill_update`, `list_schedules`, `add_schedule`, `schedule_update`, `get_permission_tier`, `tool_list`/`tool_add`/`tool_remove`, `plugin_list`/`plugin_add`/`plugin_remove`, `kv_get`/`kv_set`/`kv_delete`/`kv_list`/`kv_set_nx`, `browser_profile_list`/`browser_profile_info`/`browser_profile_clear`/`browser_profile_delete`, `persona_get`/`persona_update`, `set_fallback`, `get_cost_summary`. `skill_get` and `persona_get` are read-only (all tiers). `persona_update` for soul/user uses `applyOrSubmit` (autonomous = direct, supervised = approval, restricted = denied); memory writes directly at all tiers. In supervised mode mutation tools submit to the approval Manager rather than acting directly. `browser_profile_delete` always requires approval regardless of tier.
...
`internal/tool/manager.go` supports two MCP transports:

- **stdio** (default): spawns a subprocess, uses stdin/stdout for JSON-RPC. Existing behaviour.
- **sse**: connects to a remote Streamable HTTP/SSE endpoint. No subprocess; uses HTTP long-polling. Set `transport = "sse"` with a `url` field in `[tools.*]`.

Security hardening (`internal/tool/urlvalidate.go`, `internal/tool/envresolve.go`):
- **SSRF protection**: `validateToolURL` blocks localhost, loopback, link-local (169.254.x.x), and cloud metadata endpoints (169.254.169.254, IMDSv2). Redirect targets are re-validated.
- **HTTP header injection prevention**: header name/value sanitisation rejects newlines and forbidden header names (`Host`, `Content-Length`, `Transfer-Encoding`, `Connection`).
- **Env var denylist**: `${NAME}` placeholders in URLs and env maps are resolved at spawn time. Patterns matching `DENKEEPER_*_SECRET`, `DENKEEPER_*_PASSWORD*`, `DENKEEPER_DB_KEY`, `DENKEEPER_OIDC_CLIENT_SECRET` are refused.
- **URL/arg redaction**: `ServerInfo` response returned to API clients strips `url`, `headers`, and env values — only the `transport` and masked existence is exposed.

New `[mcp]` config section:
```toml
[mcp]
request_timeout_secs = 30    # per-request timeout (0 = no timeout)
auto_restart = true          # restart crashed stdio servers
max_restart_attempts = 3     # consecutive failures before disabling
restart_cooldown = "5m"      # reset failure counter after this uptime
url_allowlist = []           # optional wildcard list for SSE hosts
```

Per-tool override: `request_timeout_secs` in `[tools.*]` overrides the global default.
...
- MCP tool support: engine spawns MCP servers (stdio or SSE) at startup, discovers tools, passes them to the LLM, executes tool calls in an agentic loop (serial). Auto-restart for stdio servers with configurable backoff.
...
- External REST API: auth, rate limiting, CORS, TLS, health, read-only data endpoints, chat endpoint with SSE streaming, session deletion, approval CRUD, API key CRUD (runtime key management), tool/plugin CRUD (`tools:read`/`tools:write` scopes), browser profile/session/config endpoints (`browser:read`/`browser:write` scopes), schedule CRUD (`schedules:write` — create/update/delete with TOML persistence), skill CRUD (`skills:read`/`skills:write` — get/create/update/delete with file management), persona read/write (`agents:read`/`agents:write` — get/update persona sections with path traversal protection), KV store read/delete (`kv:read`/`kv:write` — list/get/delete agent-scoped keys with prefix filtering), agent config mutation (`PATCH /api/v1/agents/{name}` with `agents:write` scope — session_tier, llm_model, description, browser_url_allowlist with runtime engine mutation + TOML persistence). Agent detail endpoint exposes `tool_names`, `persona_dir`, and `persona_sections`.
- Approval workflows: TTL-based supervised approvals for user_update / soul_update / create_skill / modify_schedule / install_tool directives, with Telegram inline keyboard buttons (Approve/Deny) and keyboard auto-removal on resolution.
- Config MCP server: per-agent in-process MCP tools for skill listing/creation/get/update, schedule listing/addition/update, permission tier inspection, tool/plugin management (add/remove/list), KV store operations (get/set/delete/list/set_nx), browser profile management (list/info/clear/delete), and persona read/update (get section content, update with tier-aware approval for soul/user).
...
- Web dashboard: embedded Svelte SPA (13 pages: Login, Overview, Chat, Approvals, Sessions, Schedules, Skills, Tools, Browser, KV Store, Costs, Agents, API Keys) served via the API server. Dark mode toggle and warm light theme (persisted via `localStorage`). Chat page: state lives in a module-level Svelte store (`web/src/chatStore.js`) so it survives navigation — SSE streams keep updating even when the user switches to another page and returns; shows "Thinking..." status, inline tool-call progress, per-message token count and ~cost. API Keys page supports full CRUD. Tools page supports add/remove of MCP tools and plugins. Schedules page supports full CRUD (add/edit/delete with modal dialogs). Skills page supports full CRUD (add/edit/delete with agent selection, body textarea, async skill content fetch). Browser page shows active sessions, profile management (delete with confirmation), and read-only configuration summary. KV Store page: agent-scoped key browser with prefix filter, expandable values, and delete with confirmation. Overview page: cost breakdown table with sortable per-session costs, clickable Total Cost card links to Costs page. Costs page: per-agent breakdown table (sessions, messages, input/output tokens, cost), clickable agent drill-down to per-session detail, summary cards with token totals. Agent detail page: inline persona section viewer/editor (read/write for editable sections), config editing card (permission tier dropdown, LLM model input, description, browser URL allowlist, read-only adapters).
- E2E integration tests: build-tagged (`//go:build integration`) test suite in `internal/integration/` — boots full API server in-process with configurable mock LLM, exercises chat (JSON + SSE), auth scoping, approval flow, multi-agent routing, and schedule CRUD. Run via `just test-integration`.
- CI/CD: golangci-lint (with gosec SAST), govulncheck, cosign signing, SBOM generation, GoReleaser with .deb/.rpm/.tar.gz, Homebrew tap, Docker (ghcr.io) with SLSA provenance. Web UI is built in CI before any Go step. Integration tests run in CI after unit tests.
...
- OpenTelemetry observability: `internal/otel/` with Prometheus pull exporter (`/metrics` endpoint, no auth) and optional OTLP HTTP push exporter for traces. Global OTel providers — zero overhead when disabled (no-ops, no nil checks). Metrics: `denkeeper.llm.duration`, `denkeeper.llm.tokens`, `denkeeper.llm.cost`, `denkeeper.llm.errors` (Router), `denkeeper.messages`, `denkeeper.sessions.active`, `denkeeper.chat.duration`, `denkeeper.tool_calls` (Engine), `denkeeper.dispatch` (Dispatcher). Trace hierarchy: `dispatcher.route` → `agent.chat` → `llm.complete` with context propagation. Config: `[otel]` with `enabled`, `traces_endpoint`, `service_name` (default "denkeeper"). Env override: `DENKEEPER_OTEL_TRACES_ENDPOINT`.
- Pipeline logging: structured slog events throughout the chat pipeline — "llm response received" (finish_reason, model, content_len, token breakdown), "chat complete" (response_len, finish_reason, conversation), "tool round complete" (round N summary), per-tool execution log (duration_ms, result_len). Router logs each successful completion at debug level. Warn on empty LLM responses. SSE chat endpoint streams `tool_start`/`tool_end` events with `duration_ms` so the web Chat page can show inline tool call progress.
- Remote MCP / security: SSE transport for `[tools.*]` entries (set `transport = "sse"`, `url = "https://..."`), global `[mcp]` config section with timeout/auto-restart/cooldown/URL-allowlist. SSRF blocklist, header injection prevention, env var denylist, and URL/arg redaction in API responses. See "Remote MCP Servers" section above for details.
- OAuth2/OIDC authentication: dual auth path — Bearer token (existing API keys) + session cookie (new dashboard auth). AES-256-GCM encrypted session cookies (`internal/api/session.go`) with `HttpOnly`/`Secure`/`SameSite=Lax`. Password login with bcrypt (cost 13, `denkeeper passwd` CLI). IP-based rate limiting (5 attempts/15 min) on `POST /auth/login`. OIDC Authorization Code flow with PKCE (S256) and nonce verification (`internal/api/oidc.go`) via `go-oidc/v3`. Requires `email_verified: true` claim, case-insensitive email allowlist. CSRF protection via Origin header check. Auth routes (no auth required): `GET /auth/config`, `POST /auth/login`, `POST /auth/logout`, `GET /auth/session`, `GET /auth/oidc/login`, `GET /auth/callback`. Config: `[api.auth]` with `password_hash`, `session_secret` (hex AES key), `session_max_age`, `[api.auth.oidc]` with `issuer`, `client_id`, `client_secret`, `redirect_url`, `scopes`, `allowed_emails`. Env overrides: `DENKEEPER_API_AUTH_SESSION_SECRET`, `DENKEEPER_OIDC_CLIENT_ID`, `DENKEEPER_OIDC_CLIENT_SECRET`. Web Login page conditionally shows SSO button, password input, and collapsible API key section based on server config.
- PIN-protected account setup: first-run setup generates a one-time 6-digit PIN (`crypto/rand`, logged to stdout) when no API keys or password exist. Web dashboard setup page offers two tabs: "Create Account" (PIN + password, recommended) and "Create API Key" (existing flow). `POST /api/v1/setup/account` validates PIN (constant-time comparison via `crypto/subtle`, rate-limited 5 attempts/15 min), hashes password (bcrypt cost 13), auto-generates session secret (32 bytes `crypto/rand`), persists `password_hash` + `session_secret` to TOML via `tool.SetAuthConfig`, hot-reloads `SessionManager` in-place, creates session cookie, clears PIN. `GET /api/v1/setup` now returns `account_setup_available: bool`. `setupRequired()` also returns false when `passwordHash != ""`.
- Cost accuracy (Phase 8): `internal/llm/pricing/` package with `Registry` (exact + prefix match, longest-prefix-wins), bundled defaults for ~70 model families (Anthropic, OpenAI, Google Gemini, Meta Llama, Mistral, DeepSeek, OpenRouter-prefixed variants), `CachedPrompt` field in `TokenUsage` (populated from Anthropic `cache_read_input_tokens` and OpenAI `prompt_tokens_details.cached_tokens`), updated `TokenCost(resp, reg)` returning (cost, source) with priority: provider-reported > registry > fallback > $0. `[costs]` TOML config section with `default_rate_per_1k_tokens` (fallback rate) and `[costs.model_prices]` map for operator overrides. `pricing_source` OTel attribute on `denkeeper.llm.cost` metric. Structured log warning for unknown models.
- Supervised tool call approval (Issue #30): supervised agents now submit each MCP tool call for operator approval before executing. Blocks on `Manager.WaitForResolution`. Dispatcher sends intermediate approval prompt with inline keyboard buttons. See "Approval Workflows" section above.
- MCP server health monitoring: `StartHealthChecker(ctx, interval)` on `tool.Manager` probes stdio/SSE servers periodically (30s in production) via ListTools and auto-restarts crashed ones with exponential backoff. Respects `[mcp]` config fields `auto_restart`, `max_restart_attempts`, `restart_cooldown`. `ServerStatus` now includes `restart_count`, `last_error`, `uptime_secs` and dynamic status (`connected`/`error`/`disabled`). Tools page shows health details.
- Available models API: `GET /api/v1/models` (scope `agents:read`) queries all configured providers via optional `ModelLister` interface and returns a sorted, de-duplicated model list. Implemented for Anthropic (`/v1/models`), OpenAI/OpenRouter (`/v1/models`), and Ollama (`/api/tags`). Agent detail page model field uses a browser `<datalist>` populated from this endpoint.
- Next: Phase 8 complete except web pricing registry card. See PRD §15 for the full roadmap.
