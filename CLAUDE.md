...
- **Telegram outgoing parse mode**: `OutgoingMessage.ParseMode == ""` means "the text is LLM-authored CommonMark". `prepareOutgoingText` (`internal/adapter/telegram/telegram.go`) runs it through `renderTelegramHTML` (`render.go`, goldmark + Linkify + Strikethrough) and sends it as `parse_mode=HTML` with every text node escaped. This replaced the legacy `Markdown` parse mode, which has no intraword-emphasis rule and silently deleted `_` inside URLs/identifiers (issue #243) — Telegram *accepts* those messages, so the send-error fallback could never detect the misparse. A non-empty `ParseMode` means the caller owns its own markup (dispatcher activity log, standalone approval prompts) and is passed through untouched **and never retried**. Fallbacks for the rendered path: a render exceeding `maxMessageChars` (4096 runes) or a rejected send falls back to the unmodified source with no parse mode. Telegram supports only `b/i/u/s/a/code/pre/blockquote/tg-spoiler/tg-emoji` — any new tag in the renderer must be on that list or Telegram rejects the whole message. `EditText`/`EditMessage` are unchanged (empty parse mode there still means plain text); only in-tree caller is the activity log, which always passes `"HTML"`.
