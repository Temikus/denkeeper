...
- **`exclude_source` applies to list *and* stats**: dry-run/eval per-round events carry the *existing* `llm`/`tool_call` categories, so a category chip cannot separate them — source is the only discriminator. `audit.Store.Stats` therefore takes `StatsOpts` (not a bare `since`), and `Stats` reuses `buildWhereClause` so the exclusion can never drift from `List`; the events-last-hour window applies it too. The AuditLog page's "Show eval events" toggle passes `exclude_source=eval,dryrun` on **both** queries. The page's `categoryItems` list is hardcoded and carries an `Evals` entry for the `eval` lifecycle category.
