...
- **`requires.tools` is declared but not yet enforced**: it round-trips through parse and every CRUD surface, and `skill_get`/`GET /skills/{agent}/{name}` echo it back, but nothing reads it to gate tool exposure yet — that is finding B2 (`design/agent-ops-findings-2026-07-12.md`), which will turn it into a prompt-shrinking *and* capability-reduction control. Treat the declared set as load-bearing when wiring B2; it is already authorable and preserved.
