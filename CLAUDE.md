...
**Adapters**: Typing indicators before LLM calls, platform-native formatting, inline keyboards for approvals. Telegram adapter registers built-in commands (`/start`, `/help`, `/stop`, `/panic`, `/resume`, `/debug`) plus skill `command:` triggers via `setMyCommands` on startup (`RegisterSkillCommands`).
