...
- **Engine knobs**: `Engine.SetMaxContextMessages` (default 50) caps loaded history; `Engine.SetApprovalConfig` controls approval retries; `Engine.SetSupervisorConfig` sets supervisor timeout (default 30s) and context message count (default 5); `Engine.SetMaxToolRounds` caps the tool-call loop. All knobs are re-applied to live engines on hot-reload (`buildReloadFunc` iterates `cfg.Agents`). Stalled SSE streams are capped by `IdleTimeoutReader`.
...
- **External MCP skill tools** (`internal/mcpserver/tools_skills.go`): `skill_create`, `skill_update`, `skill_delete` persist to disk via `configmcp.ApplySkill*` (disk-first, then in-memory) and return a `toolError` on persist failure — in-memory state is left unchanged. `skill_update` supports optional `version` and `new_name` (rename) fields. `validSkillName` rejects names with path separators or `..` (friendly outer guard).
- **Skill-file IO is confined to the skills dir via `os.Root`**: all skill writes/removes in `configmcp` (`ApplySkillCreate/Update/Rename`, `RemoveSkillFile`) go through an `os.Root` opened on `agentSkillsDir` (`openSkillRoot` + `writeSkillFileAtomic`). The OS-level path resolution refuses any write or remove that escapes the directory — `..` traversal or escaping symlinks — a hard boundary that backstops name validation and covers all three surfaces (REST, in-process config MCP, external MCP). Writes are temp-file + `Root.Rename` for atomicity. Requires Go ≥ 1.24 (`os.OpenRoot`); `Root.WriteFile`/`Rename`/`Remove` need ≥ 1.25.
