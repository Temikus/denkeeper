...
func Parse(data []byte) (*Config, error) {
	...
	// First try a direct unmarshal. A failure may be a config generated by
	// Helm's toToml, which renders YAML integers as TOML floats (e.g.
	// allowed_users = [3.87e+08]) that won't decode into int fields. Rather
	// than match go-toml's internal, version-specific error wording (which
	// changed in v2.4.0), unconditionally try the normalisation fallback:
	// normaliseFloatsToInts only rewrites whole-number floats to ints and is a
	// no-op for everything else, so it cannot corrupt an otherwise-valid
	// config. If the failure was unrelated, normalisation either errors (e.g.
	// a syntax error) or the retry re-fails — either way we surface the
	// original error.
...
func validateEval(e *EvalConfig) error {
	...
	if e.JudgeMaxCostPerRun <= 0 {
		return fmt.Errorf("judge_max_cost_per_run must be greater than 0, got %v", e.JudgeMaxCostPerRun)
	}
