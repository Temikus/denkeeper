...
func TestExecuteToolCallDeduped_CachedHit_SkipsExecution(t *testing.T) {
	...
	first, firstRecord, _ := e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	if firstRecord.Outcome != "ok" {
		t.Fatalf("first Outcome = %q, want ok", firstRecord.Outcome)
	}
	second, record, _ := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestExecuteToolCallDeduped_NonIdempotent_ReExecutes(t *testing.T) {
	...
	_, r1, _ := e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	_, r2, _ := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestExecuteToolCallDeduped_CacheKey_ArgSensitive(t *testing.T) {
	...
	_, _, _ = e.executeToolCallDeduped(context.Background(), tcA, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	_, record, _ := e.executeToolCallDeduped(context.Background(), tcB, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestCachedToolCallResult_EventsAndAudit(t *testing.T) {
	...
	_, _, _ = e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, capture, state)
	events = nil
	_, _, _ = e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, capture, state)
