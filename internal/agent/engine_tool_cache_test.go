...
func TestExecuteToolCallDeduped_CachedHit_SkipsExecution(t *testing.T) {
	...
	first, firstRecord := e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	if firstRecord.Outcome != "ok" {
		t.Fatalf("first Outcome = %q, want ok", firstRecord.Outcome)
	}
	second, record := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestExecuteToolCallDeduped_NonIdempotent_ReExecutes(t *testing.T) {
	...
	_, r1 := e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	_, r2 := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestExecuteToolCallDeduped_CacheKey_ArgSensitive(t *testing.T) {
	...
	_, _ = e.executeToolCallDeduped(context.Background(), tcA, 1, "conv:1", false, turnRun{grant: grantAll}, nil, state)
	_, record := e.executeToolCallDeduped(context.Background(), tcB, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestExecuteToolCallDeduped_CachedHit_SkipsApproval(t *testing.T) {
	...
	result, record := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", true,
		turnRun{grant: grantAll}, func(ev ChatEvent) { events = append(events, ev) }, state)
...
func TestExecuteToolCallDeduped_DeniedThenNeverCached(t *testing.T) {
	...
	_, record := e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, nil, state)
...
func TestCachedToolCallResult_EventsAndAudit(t *testing.T) {
	...
	_, _ = e.executeToolCallDeduped(context.Background(), tc, 1, "conv:1", false, turnRun{grant: grantAll}, capture, state)
	events = nil
	_, _ = e.executeToolCallDeduped(context.Background(), tc, 2, "conv:1", false, turnRun{grant: grantAll}, capture, state)
